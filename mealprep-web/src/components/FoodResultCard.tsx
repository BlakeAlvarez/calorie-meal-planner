...
export function FoodResultCard({
  ...
  onClose: () => void;
    ...
    function getNutrientValue(nutrients: Nutrient[], key: string | number): number {
  ...
  return (
    <Card className="flex-1 min-w-[300px] max-w-[600px]">
      <CardHeader>
        <CardTitle className="text-lg font-semibold">{food.description}</CardTitle>
        <CardDescription>
          {food.brandOwner ? `Brand: ${food.brandOwner}` : "Generic Item"}
        </CardDescription>
      </CardHeader>
