...
export function SaveMealButton() {
  ...
  const handleSave = async (name: string, createdBy: string, notes: string) => {
    ...
    const resolvedPlans = plans.map((plan) => {
      ...
      const kcalPerUnit = gramsPerUnit > 0 ? (gramsPerUnit / 100) * kcalPer100g : 0;
    ...
    const totalCalories = resolvedPlans.reduce((sum, p) => sum + (p.kcal ?? 0), 0);

    const mergedGroups = groups.map((group) => {
      const enrichedIngredients = group.ingredients.map((ingredient) => {
        const plan = resolvedPlans.find((p) => p.foodId === ingredient.foodId);
        return {
          ...ingredient,
          ...
          percent:
            plan?.mode === "percent"
              ...
              ? Math.round((plan.kcal / totalCalories) * 1000) / 10
              : 0,
        };
      });
      ...
      return {
        ...
        totalCalories: enrichedIngredients.reduce((sum, ing) => sum + (ing.kcal ?? 0), 0),
      };
    });
    ...
    try {
      ...
      toast.success("Meal saved!", {
        ...
        action: {
          label: "Copy Link",
          onClick: () => {
            navigator.clipboard.writeText(`/shared/meal/${data.id}`);
          },
        },
      });
    } catch (e) {
