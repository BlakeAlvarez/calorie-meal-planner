...
export function MealItemCard({ food, groupId }: { food: Food; groupId?: string }) {
  ...
  const removeIngredientFromGroup = useGroupStore((s) => s.removeIngredientFromGroup);
  ...
  const handleSave = () => {
    ...
    if (existing) {
      ...
      const kcal = isUnitBased
        ? editedCalories
        : Math.round((editedGrams / 100) * (kcalPer100g ?? 0) * 10) / 10;
      ...
      setPlan(food.fdcId, existing.mode, existing.value, grams, kcal, percentValue);
    }
  ...
  return (
    <Card ref={setNodeRef} style={style} {...listeners} {...attributes} className="w-full max-w-md mx-auto">
      ...
      <CardHeader>
        {isEditing ? (
          ...
          <CardTitle className="text-base font-semibold">{food.description}</CardTitle>
        )}
      </CardHeader>
            ...
            {isUnitBased ? (
              ...
              <>
                ...
                <Input
                  ...
                  className="w-16"
                  value={editedGrams}
                  onChange={(e) => setEditedGrams(parseFloat(e.target.value) || 1)}
                  placeholder="g"
                />
                <span>= {kcalPer100g?.toFixed(1)} kcal/100g</span>
              </>
            )}
          </div>
        ) : (
      ...
      <CardFooter className="flex justify-between items-center">
        <Button
          ...
          size="sm"
          onClick={() => {
            const { removeIngredientFromGroup } = useGroupStore.getState();
