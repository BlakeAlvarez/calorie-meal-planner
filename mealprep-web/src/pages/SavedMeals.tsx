    ...
    <div className="p-6 space-y-4 overflow-y-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pt-2">
        {filtered.map((meal) => {
          ...
          return (
            <div key={meal.id} className="border p-4 rounded space-y-2 shadow-sm">
              <div>
              ...
              {meal.peopleCount != null && (
                <p className="text-sm text-muted-foreground">
                  {meal.peopleCount} {meal.peopleCount === 1 ? "person" : "people"},{" "}
              ...
              <Accordion type="single" collapsible className="w-full">
                <AccordionItem value={`item-${meal.id}`}>
                  <AccordionTrigger className="text-sm">View Ingredients</AccordionTrigger>
                          ...
                          <Table>
                            <TableHeader>
                              <TableRow>
                                ...
                                <TableHead>% of Meal</TableHead>
                              </TableRow>
                            </TableHeader>
                            <TableBody>
                              {group.ingredients.map((ing: any) => {
                                const food = foods.find((f: any) => f.fdcId === ing.foodId) as any;
                                const name = food?.description || ing.description || ing.name || "Unnamed";

                                return (
                                  <TableRow key={ing.foodId}>
                                    <TableCell>{name}</TableCell>
                                    <TableCell>{ing.grams?.toFixed(1) ?? "-"}</TableCell>
                                    <TableCell>{ing.kcal?.toFixed(1) ?? "-"}</TableCell>
              ...
              {meal.notes && (
                <div className="mt-2 space-y-1">
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => setOpenNotesMealId(meal.id)}
                  >
                    View Full Notes
                  </Button>
