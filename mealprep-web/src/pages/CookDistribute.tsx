...
export default function CookDistribute() {
  ...
  const TOTAL_TARGET_CALORIES = plans.reduce((sum, plan) => sum + (plan.kcal ?? 0), 0);
  ...
  plans.forEach(plan => {
    ...
    const grams = !isUnit && kcalPer100g > 0 ? Math.round((rawKcal / kcalPer100g) * 1000) / 10 : 0;

    foodIdToResolved.set(plan.foodId, { kcal, grams, isUnit, kcalPerUnit });
  });

  // determine ungrouped foods (not assigned to any group)
  const groupedFoodIds = groups.flatMap((g) => g.ingredients.map((i) => i.foodId));
  const ungroupedFoods = foods.filter((f) => !groupedFoodIds.includes(f.fdcId));
  const [ungroupedCookedWeights, setUngroupedCookedWeights] = useState<Record<number, number>>({});
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <h1 className="text-2xl font-bold text-center flex-1">Cook & Distribute</h1>
        ...
        return (
          <div className="flex justify-center" key={group.id}>
            <div className="w-full max-w-2xl border p-4 rounded space-y-2">
              ...
              <p className="text-sm text-muted-foreground">
                Initial Weight: {getRawWeight(group).toFixed(1)} g
              </p>
              <div className="flex items-center gap-4">
                <Label htmlFor={`cooked-${group.id}`}>Cooked Weight (g)</Label>
                <Input
                  id={`cooked-${group.id}`}
                  type="number"
                  className="w-32"
                  value={group.cookedWeightGrams ?? ""}
                  onChange={(e) => setCookedWeight(group.id, parseFloat(e.target.value) || 0)}
                />
              </div>
                ...
                <Table className="mt-2">
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="w-1/3">Grams / Meal × Meals</TableHead>
                      <TableHead className="w-1/3 text-right">Calories / Meal</TableHead>
                      <TableHead className="w-1/3 text-right">Total Grams</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {results.map((res) => {
                      ...
                      return (
                        <TableRow key={res.personId}>
                          ...
                          <TableCell>
                            {res.adjustedGramsPerMeal.toFixed(1)} g / meal × {person?.meals} meals
                          </TableCell>
                          <TableCell className="text-right text-muted-foreground">
                            {res.adjustedKcalPerMeal.toFixed(1)} kcal / meal
                          </TableCell>
                          <TableCell className="text-right text-muted-foreground">
                            {res.adjustedGramsTotal.toFixed(1)} g total
                          </TableCell>
                        </TableRow>
                      );
                    })}
      ...
      {ungroupedFoods.length > 0 && (
        <div className="space-y-6 pt-2">
          <h2 className="text-lg font-semibold text-center">Ungrouped Foods</h2>
          ...
          {ungroupedFoods.map((food) => {
            ...
            return (
              <div className="flex justify-center" key={food.fdcId}>
                <div className="w-full max-w-2xl border p-4 rounded space-y-2">
                  <h3 className="font-semibold text-base">{food.description}</h3>

                  {!food.unitLabel && (
                    <p className="text-sm text-muted-foreground">
                      Raw Weight: {rawGrams.toFixed(1)} g ({(rawGrams / 453.592).toFixed(2)} lb)
                    </p>
                  )}
                  <div className="flex items-center gap-4">
                    <Label htmlFor={`ungrouped-${food.fdcId}`}>Cooked Weight (g)</Label>
                    <Input
                      id={`ungrouped-${food.fdcId}`}
                      type="number"
                      className="w-32"
                      value={ungroupedCookedWeights[food.fdcId] ?? food.cookedWeightGrams ?? ""}
                      onChange={(e) => {
                        const value = parseFloat(e.target.value) || 0;
                        setUngroupedCookedWeights(prev => ({ ...prev, [food.fdcId]: value }));
                        useMealStore.getState().updateFood(food.fdcId, { cookedWeightGrams: value });
                      }}
                    />
                  </div>
                  ...
                  {cookedGrams > 0 && (
                    <>
                      <p className="text-sm text-muted-foreground">
                        Total Calories: {resolved.kcal.toFixed(1)} kcal
                      </p>
