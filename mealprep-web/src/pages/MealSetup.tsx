    ...
    <div className="p-6 max-w-xl mx-auto space-y-6">
      ...
      {people.length > 0 && step === "who" && (
        <div className="border p-4 rounded bg-muted text-muted-foreground text-sm mb-4 space-y-2">
          <p className="font-medium text-black">Existing Plan Detected:</p>
          {people.map((p) => (
            <p key={p.id}>
      ...
      {step === "who" && (
        <div className="space-y-4">
          ...
          <div className="flex gap-4">
            <Button onClick={() => setStep("solo_meals")}>Just Me</Button>
            <Button onClick={() => setStep("multi_count")}>More Than Just Myself</Button>
          </div>
        </div>
      )}
      ...
      {step === "solo_meals" && (
        <div className="space-y-4">
          <div className="space-y-2">
            <Label className="block pb-1">How many meals per day?</Label>
            <Input
              type="number"
              value={mealsPerDay}
          ...
          <div className="flex gap-4">
            <Button variant="secondary" onClick={() => setStep("who")}>Back</Button>
            <Button onClick={() => setStep("solo_days")}>Next</Button>
          </div>
        </div>
      )}
      ...
      {step === "solo_days" && (
        <div className="space-y-4">
          <div className="space-y-2">
            <Label className="block pb-1">How many days are you planning for?</Label>
            <Input
              type="number"
              value={numDays}
          ...
          <div className="flex gap-4">
            <Button variant="secondary" onClick={() => setStep("solo_meals")}>Back</Button>
            <Button onClick={() => setStep("solo_kcal")}>Next</Button>
          </div>
        </div>
      )}
      ...
      {step === "solo_kcal" && (
        <div className="space-y-4">
          <div className="space-y-2">
            <Label className="block pb-1">How many calories per meal?</Label>
            <Input
              type="number"
              value={caloriesPerMeal}
              onChange={(e) => setCaloriesPerMeal(parseInt(e.target.value) || 0)}
            />
          </div>
          <div className="flex gap-4">
            <Button variant="secondary" onClick={() => setStep("solo_days")}>Back</Button>
            <Button onClick={handleSoloComplete}>Start Building</Button>
          </div>
        </div>
      )}
      ...
      {step === "multi_count" && (
        <div className="space-y-4">
          <div className="space-y-2">
            <Label className="block pb-1">How many people are you prepping for?</Label>
            <Input
              type="number"
              value={numPeople}
      ...
      {step === "multi_info" && (
        <div className="space-y-4">
          ...
          {peopleInputs.map((person, index) => (
            <div key={person.id} className="border p-4 rounded space-y-4">
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    const updated = [...peopleInputs];
                    updated[index].meals = parseInt(e.target.value) || 1;
                    setPeopleInputs(updated);
                  }}
                />
              </div>
              <div className="space-y-2">
                <Label className="block pb-1">Calories per meal</Label>
                <Input
                  type="number"
                  value={person.targetCalories}
                  onChange={(e) => {
                    const updated = [...peopleInputs];
                    updated[index].targetCalories = parseInt(e.target.value) || 0;
                    setPeopleInputs(updated);
                  }}
                />
              </div>
          ...
          <div className="flex gap-4">
            <Button variant="secondary" onClick={() => setStep("multi_count")}>Back</Button>
            <Button onClick={handleMultiComplete}>Start Building</Button>
          </div>
        </div>
      )}
    </div>
