...
interface IngredientPlanState {
  ...
  setPlan: (foodId: number, mode: IngredientPlanMode, value: number, grams: number, kcal: number, percent: number) => void;
  ...
  persist(
    ...
    (set) => ({
      ...
      setPlan: (foodId, mode, value, grams, kcal, percent) =>
        set((state) => {
          const existing = state.plans.find((p) => p.foodId === foodId);
          const updated = { foodId, mode, value, grams, kcal, percent };
          return {
            plans: existing
              ? state.plans.map((p) => (p.foodId === foodId ? updated : p))
              : [...state.plans, updated],
          };
      }),
